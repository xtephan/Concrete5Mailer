...
class DashboardMailSmtpSettingsTestSettingsController extends DashboardBaseController {
    ...
    public function test() {
        ...
        elseif(!Loader::helper('validation/strings')->email($mailRecipient, true)) {
            ...
            try {
                ...
                $mail->setBody($body);
                $mail->sendMail();
                $this->redirect('/dashboard/mail/smtp_settings/test_settings/', 'successful', rawurlencode($mailRecipient));
            }
            catch(Exception $x) {
